...
// finite state machine initilization
int fsm_A_up_down = 1;
int fsm_B_up_down = 2;
int fsm = 1;
...
void loop(const mjModel* m, mjData* d)
{
    ...
    static int flag = 1;
    static int state = 1;
    double theta_FR_thigh_ctrl = 0;
    double theta_FL_thigh_ctrl = 0;
    double theta_RR_thigh_ctrl = 0;
    double theta_RL_thigh_ctrl = 0;
    double theta_FR_calf_ctrl = 0;
    double theta_FL_calf_ctrl = 0;
    double theta_RR_calf_ctrl = 0;
    double theta_RL_calf_ctrl = 0;

    // march motion parameters initialization
    static double step_time = 2;
    double t0 = 0;
    double tf = 0.15;            // this is half the each step of time --- tune the march motion speed here
    double y0_up = 0.280107;     // start point of the up motion trajectory
    double yf_up = 0.180165;     // enddd point of the up motion trajectory
    double y0_down = 0.180165;   // start point of the down motion trajectory
    double yf_down = 0.280107;   // enddd point of the down motion trajectory
    ...
    {
        // A stay
        theta_FL_thigh_ctrl = 0.795452;
        theta_FL_calf_ctrl = -1.590904;
        theta_RR_thigh_ctrl = 0.795452;
        theta_RR_calf_ctrl = -1.590904;

        // B stay
        theta_FR_thigh_ctrl = 0.795452;
        theta_FR_calf_ctrl = -1.590904;
        theta_RL_thigh_ctrl = 0.795452;
        theta_RL_calf_ctrl = -1.590904;
    }
    
    // simulation starts from here
    if (d->time > 2)
    {
        // state machine define here
        // define A = FL + RR
        // define B = FR + RL

        // transition
        if (pos_FL_foot[2]<0.02 && pos_RR_foot[2]<0.02 && fsm==fsm_A_up_down) // hanging = 0.319 on_ground = 0.02
        {
            fsm = fsm_B_up_down;
            step_time = d->time;
        }
        if (pos_FR_foot[2]<0.02 && pos_RL_foot[2]<0.02 && fsm==fsm_B_up_down)
        {
            fsm = fsm_A_up_down;
            step_time = d->time;
        }
        ...
        // actions
        if (fsm == fsm_A_up_down) // A_up_down: trajectory generation & analytic method ik
        {
            if (d->time-step_time <= tf) // A up
            {
                // extract from matlab
                double a0_up = (yf_up*t0*t0*(3*tf-t0) + y0_up*tf*tf*(tf-3*t0))/((tf-t0)*(tf-t0)*(tf-t0));
                double a1_up = 6*t0*tf*(y0_up-yf_up)/((tf-t0)*(tf-t0)*(tf-t0));
                double a2_up = 3*(t0+tf)*(yf_up-y0_up)/((tf-t0)*(tf-t0)*(tf-t0));
                double a3_up = 2*(y0_up-yf_up)/((tf-t0)*(tf-t0)*(tf-t0));

                double x = 0;
                double y = a0_up + a1_up*(d->time-step_time) + a2_up*(d->time-step_time)*(d->time-step_time) + a3_up*(d->time-step_time)*(d->time-step_time)*(d->time-step_time);
                ...
                theta_FL_thigh_ctrl = -theta1;
                theta_FL_calf_ctrl = -theta2;
                theta_RR_thigh_ctrl = -theta1;
                theta_RR_calf_ctrl = -theta2;
            }

            if (d->time-step_time > tf && d->time-step_time <2*tf) // A down
            {
                // extract from matlab
                double a0_down = (yf_down*t0*t0*(3*tf-t0) + y0_down*tf*tf*(tf-3*t0))/((tf-t0)*(tf-t0)*(tf-t0));
                double a1_down = 6*t0*tf*(y0_down-yf_down)/((tf-t0)*(tf-t0)*(tf-t0));
                double a2_down = 3*(t0+tf)*(yf_down-y0_down)/((tf-t0)*(tf-t0)*(tf-t0));
                double a3_down = 2*(y0_down-yf_down)/((tf-t0)*(tf-t0)*(tf-t0));

                double x = 0;
                double y = y = a0_down + a1_down*(d->time-step_time-tf) + a2_down*(d->time-step_time-tf)*(d->time-step_time-tf) + a3_down*(d->time-step_time-tf)*(d->time-step_time-tf)*(d->time-step_time-tf);
        ...
        if (fsm == fsm_B_up_down)  // B_up_down: trajectory generation & analytic method ik
        {
            if (d->time-step_time <= tf) // B up
            {
                // extract from matlab
                double a0_up = (yf_up*t0*t0*(3*tf-t0) + y0_up*tf*tf*(tf-3*t0))/((tf-t0)*(tf-t0)*(tf-t0));
                double a1_up = 6*t0*tf*(y0_up-yf_up)/((tf-t0)*(tf-t0)*(tf-t0));
                ...
                theta_FR_thigh_ctrl = -theta1;
                theta_FR_calf_ctrl = -theta2;
                theta_RL_thigh_ctrl = -theta1;
                theta_RL_calf_ctrl = - theta2;
            }

            if (d->time-step_time > tf && d->time-step_time < 2*tf) // B down
            {
                // extract from matlab
                double a0_down = (yf_down*t0*t0*(3*tf-t0) + y0_down*tf*tf*(tf-3*t0))/((tf-t0)*(tf-t0)*(tf-t0));
                double a1_down = 6*t0*tf*(y0_down-yf_down)/((tf-t0)*(tf-t0)*(tf-t0));
    ...
    control_loop(m,d,
                 q,u,base_x,base_quat,base_angvel,base_linacc,base_linvel,
                 pos_FR_shoulder,pos_FL_shoulder,pos_RR_shoulder,pos_RL_shoulder,
                 pos_FR_elbow,pos_FL_elbow,pos_RR_elbow,pos_RL_elbow,
                 pos_FR_foot,pos_FL_foot,pos_RR_foot,pos_RL_foot,robot_com,tau,flag,state,t,
                 theta_FR_thigh_ctrl, theta_FR_calf_ctrl,
                 theta_FL_thigh_ctrl, theta_FL_calf_ctrl,
                 theta_RR_thigh_ctrl, theta_RR_calf_ctrl,
                 theta_RL_thigh_ctrl, theta_RL_calf_ctrl);
